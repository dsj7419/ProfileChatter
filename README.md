...
![ProfileChatter Demo](https://raw.githubusercontent.com/dsj7419/ProfileChatter/main/dist/profile-chat.svg?ts=1748088421)
...
| `TWITTER_BEARER_TOKEN` | ☐ (Advanced, High Cost) | Fetch Twitter/X follower count |
...
### 🌤️ AccuWeather API Setup

To enable weather integration in ProfileChatter:
...
**Important:** Due to significant Twitter API costs, live follower fetching is **disabled by default**. ProfileChatter now uses manually entered follower counts.

#### Default Behavior (Recommended)

- The `{twitterFollowers}` placeholder uses your manually entered follower count
- Configure this via the Configurator UI's "Twitter Followers (Manual)" field
...
1. **`profileChatterConfig.json`** (if present and contains the relevant section) - Created by the Configurator UI
2. **`src/config/config.js`** - Default configuration file
3. **`data/chatData.json`** - Default chat messages (used only if `profileChatterConfig.json` doesn't exist or lacks a `chatMessages` array)
...
```markdown
![My Profile Chat](https://raw.githubusercontent.com/<you>/ProfileChatter/main/dist/profile-chat.svg?ts=1748088421)
```
