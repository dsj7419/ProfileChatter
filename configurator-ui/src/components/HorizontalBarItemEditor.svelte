<script>
  ...
  function handleDrop(event, index) {
    ...
    // Clone the items array to avoid mutation
  ...
  function handleDragEnd(event) {
    ...
    // Remove drag-over styling from all item rows
    document.querySelectorAll('.chart-item-row').forEach(element => {
  ...
  {#if chartData?.items && Array.isArray(chartData.items)}
    ...
    {:else}
      ...
      {#each chartData.items as item, itemIndex}
        <div class="grid grid-cols-12 gap-2 mb-2 items-center bg-gray-50 p-2 rounded chart-item-row"
             on:dragover={(e) => handleDragOver(e, itemIndex)}
             on:dragenter={handleDragEnter}
             on:dragleave={handleDragLeave}
             on:drop={(e) => handleDrop(e, itemIndex)}
             class:drag-over={dragOverIndex === itemIndex && draggedIndex !== itemIndex}
             class:dragging={draggedIndex === itemIndex}
             role="listitem"
             aria-grabbed={draggedIndex === itemIndex}>
          
          <!-- Drag handle -->
          <div class="col-span-1 flex items-center justify-center cursor-move"
               draggable="true"
               on:dragstart={(e) => handleDragStart(e, itemIndex)}
...
.chart-item-row.drag-over {
  border: 2px dashed #4f46e5;
  padding: 6px; /* Adjust for the border */
}

.chart-item-row.dragging {
  opacity: 0.6;
