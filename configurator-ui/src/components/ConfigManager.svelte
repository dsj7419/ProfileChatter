...
<script>
  ...
  $: if (
    oauthStatuses?.github?.authenticated && 
    oauthStatuses?.github?.valid && 
    ...
    try {
      ...
      const res = await fetch(`${previewServer}/api/github/save-config`, {
        ...
        body: JSON.stringify({
          ...
          filePath,
          commitMessage,
          configContent: JSON.stringify(getPreviewConfiguration(), null, 2)
        })
      });
...
<!-- Save Configuration to GitHub section -->
<div class="p-3 border border-gray-200 rounded-md bg-white mt-6">
  <h3 class="text-sm font-medium text-gray-700 mb-3">Save Configuration to GitHub</h3>

  {#if oauthStatuses?.github?.authenticated && oauthStatuses?.github?.valid}
  ...
  {:else}
    ...
    <button
      type="button"
      on:click={() => connectProvider('github')}
