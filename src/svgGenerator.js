...
const PADDING_Y = 14 // Increased vertical padding for better text fit
const MESSAGE_MARGIN_BOTTOM = 15 // Increased margin between bubbles for clear separation
...
const TEXT_Y_BASELINE = PADDING_Y + 12 // Initial text baseline position
...
function calculateTextDimensions(text) {
  ...
  // For multi-line messages, use the number of lines * line height + padding
  // For single-line messages, use minimum height
  const textHeight = lineCount * LINE_HEIGHT
  const bubbleHeight = Math.max(MIN_BUBBLE_HEIGHT, textHeight + (PADDING_Y * 2))
...
function createMessageElement(message, positionInfo, delay) {
  ...
  // Add text element with precise positioning
  messageString += `
    <text class="${textClass}" text-anchor="${textAnchor}">
  `
  
  // Handle text content (single or multi-line)
  if (lineCount === 1) {
    // Single line text - position in middle of bubble
    const textY = bubbleHeight / 2 + 6 // Center vertically with adjustment
    messageString += `<tspan x="${textX}" y="${textY}">${message.text}</tspan>`
  } else {
    // First line
    const firstLineY = TEXT_Y_BASELINE
    messageString += `<tspan x="${textX}" y="${firstLineY}">${lines[0]}</tspan>`
...
export function generateSVG(chatData, dynamicData = {}) {
  ...
  /* ----------- PASS 1: ELEMENT CALCULATION PHASE ----------- */
  
  // Calculate dimensions and positions for all elements
  ...
  for (let i = 0; i < chatData.length; i++) {
    ...
    // Add message element details with exact position
    elementsToRender.push({
      type: 'message',
      yPos: currentY,
      message: message,
    ...
    // Calculate next position after this message
